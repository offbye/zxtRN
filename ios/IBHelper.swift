...
class IBHelper {
    ...
    class func pushStoryBoard(navi:UINavigationController?,storyBoard:StoryboardNames, controller:VCStoryboardIdentifier) {
        IBHelper.pushStoryBoard(navi, storyBoard: storyBoard.rawValue, controller: controller.rawValue)
    }
    
    class func pushStoryBoard(navi:UINavigationController?,storyBoard:String, controller:String) {
        if let navi = navi {
            let storyBoard = UIStoryboard(name: storyBoard, bundle: nil)
            let controller = storyBoard.instantiateViewControllerWithIdentifier(controller)
